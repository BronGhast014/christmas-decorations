################################################################################
# Authors: BronGhast014
# Version: 1.16.x +
# Copyright November / 2020
################################################################################

scoreboard players set @s[nbt={SelectedItem:{tag:{mug:1b}}}] BGCD_item 1
scoreboard players set @s[nbt={SelectedItem:{tag:{tray:1b}}}] BGCD_item 2
scoreboard players set @s[nbt={SelectedItem:{tag:{miniTree:1b}}}] BGCD_item 3

scoreboard players set @s[nbt={SelectedItem:{tag:{ginger_bread:1b}}}] BGCD_item 6
scoreboard players set @s[nbt={SelectedItem:{tag:{candy:1b}}}] BGCD_item 7

scoreboard players set @s[nbt={SelectedItem:{tag:{cmasTool:1b}}}] BGCD_item 11
